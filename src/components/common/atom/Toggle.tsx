...
const Toggle = ({ className = "bg-transparent", on, onToggle, leftContent, rightContent }: { className?: string; on: boolean; onToggle: () => void; leftContent: React.ReactNode; rightContent: React.ReactNode }) => {
  return (
    <div className={`${className} absolute right-6 top-0 w-[76px] h-10 flex items-center rounded-full py-2 px-0 border border-white cursor-pointer mt-4`} onClick={onToggle}>
