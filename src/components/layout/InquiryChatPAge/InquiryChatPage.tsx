      ...
      <div className="responsive_device_container bg-transparent w-full h-full z-10 pt-[3.25rem]">
        <div className="responsive_device_chat shrink-0 relative">
          {isLoading ? (
            ...
            <>
              <BotMessage message={botMessage} scrollToBottom={scrollToBottom} audioRef={audioRef} messagesEndRef={messagesEndRef} />
            </>
          )}
        ...
        <div className="responsive_device_input flex items-start grow min-h-0 overflow-y-auto relative text-base">
          <InquryInput inputValue={inputValue} setInputValue={setInputValue} handleSendMessageStream={handleSendMessageStream} isLoading={isLoading} />
        </div>
      </div>
    </>
  );
